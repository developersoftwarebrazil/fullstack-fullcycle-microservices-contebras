...
func (vc *VideoConverter) HandleMessage(d amqp.Delivery, conversionExch, confirmationKey, confirmationQueue string) {
	var task VideoTask
...
func (vc *VideoConverter) logError(task VideoTask, message string, err error) {
	...
	serializerError, _ := json.Marshal(errorData)
	slog.Error("Processing error", slog.String("error_details", string(serializerError)))
